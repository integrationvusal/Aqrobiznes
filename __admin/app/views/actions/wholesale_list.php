...
$(document).ready(function() {
	...
	$('#tablesorter').sortable({
		...
		update: function(e, ui){
			...
			$.ajax({
				url: "<?=view::create_url('wholesale', 'ajax_sort')?>",
				async: false,
				cache: false,
				method: 'POST',
				data: {
...
// ]]>
	...
	<form action="<?=view::create_url('wholesale', 'delete') ?>" method="post" id="formDeleteItem">
	...
	<section class="content-header">
		<h1>
			<?=CMS::t('menu_item_wholesale_list');?>
	...
	<section class="contextual-navigation">
		<nav>
			<?php if (CMS::hasAccessTo('wholesale/add', 'write')) { ?>
			<a href="<?=view::create_url('wholesale', 'add', ['return' => $link_return]) ?>&amp;<?=time();?>" class="btn btn-default"><i class="fa fa-plus-circle" aria-hidden="true"></i> <?=CMS::t('menu_item_wholesale_add');?></a>
			<?php } ?>
		</nav>
	</section>
	...
	<section class="content">
		...
		<div class="box">
			<div class="box-header with-border">
				<h3 class="box-title"><?=CMS::t('wholesale_list_details', [
					'{count}' => $count,
				...
				<div class="box-tools pull-right col-sm-5 col-lg-6">
					<form action="" method="get" id="formSearchAndFilter">
						...
						<div class="input-group has-feedback">
							<input type="text" name="q" value="<?=utils::safeEcho(@$_GET['q'], 1);?>" placeholder="<?=CMS::t('wholesale_search');?>" class="form-control input-md" onfocus="this.value='';" />
            ...
            <div class="box-body">
				...
				<table class="table table-bordered table-striped">
					<thead>
						<tr>
							<th><?=CMS::t('article_publish_date_placeholder');?></th>
							<th><?=CMS::t('wholesale_product');?></th>
					...
					<tbody id="tablesorter" data-page="<?=$current-1?>" data-limit="<?=$limit?>">
						...
						?>
							<tr data-id="<?=$data['id']?>">
							    ...
							    <td><?=$data['product']?></td>
								<td>
									<?php if (CMS::hasAccessTo('wholesale/edit', 'write')) { ?>
