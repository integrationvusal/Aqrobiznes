...
$(document).ready(function() {
	...
	$('#tablesorter').sortable({
		...
		update: function(e, ui){
			...
			$.ajax({
				url: "<?=view::create_url('questions', 'ajax_sort')?>",
				async: false,
				cache: false,
				method: 'POST',
				data: {
...
// ]]>
	...
	<form action="<?=view::create_url('questions', 'delete') ?>" method="post" id="formDeleteItem">
	...
	<section class="content-header">
		<h1>
			<?=CMS::t('menu_item_questions_list');?>
	...
	<section class="content">
		...
		<div class="box">
			<div class="box-header with-border">
				<h3 class="box-title"><?=CMS::t('questions_list_details', [
					'{count}' => $count,
				...
				<div class="box-tools pull-right col-sm-5 col-lg-6">
					<form action="" method="get" id="formSearchAndFilter">
						...
						<div class="input-group has-feedback">
							<input type="text" name="q" value="<?=utils::safeEcho(@$_GET['q'], 1);?>" placeholder="<?=CMS::t('questions_search');?>" class="form-control input-md" onfocus="this.value='';" />
            ...
            <div class="box-body">
				...
				<table class="table table-bordered table-striped">
					<thead>
						<tr>
							<th><?=CMS::t('questions_question');?></th>
							<th><?=CMS::t('questions_is_answer');?></th>
					...
					<tbody id="tablesorter" data-page="<?=$current-1?>" data-limit="<?=$limit?>">
						...
						?>
							<tr data-id="<?=$data['id']?>">
							    <td><?=$data['question']?></td>
							    <td><?=!empty($data['answer'])?'Bəli':'Xeyr'?></td>
								<td>
									<?php if (CMS::hasAccessTo('questions/edit', 'write')) { ?>
										<a href="<?=view::create_url('questions', 'edit', ['id'=>$data['id']])?>" title="<?=CMS::t('edit');?>">
											<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
										</a>
									<?php } ?>

									<?php if (CMS::hasAccessTo('questions/delete', 'write')) { ?>
