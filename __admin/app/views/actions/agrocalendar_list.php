...
$(document).ready(function() {
	...
	$('#tablesorter').sortable({
		...
		update: function(e, ui){
			...
			$.ajax({
				url: "<?=view::create_url('agrocalendar', 'ajax_sort')?>",
				async: false,
				cache: false,
				method: 'POST',
				data: {
...
// ]]>
	...
	<form action="<?=view::create_url('agrocalendar', 'delete') ?>" method="post" id="formDeleteItem">
	...
	<section class="content-header">
		<h1>
			<?=CMS::t('menu_item_agrocalendar_list');?>
	...
	<section class="contextual-navigation">
		<nav>
			<?php if (CMS::hasAccessTo('agrocalendar/add', 'write')) { ?>
			<a href="<?=view::create_url('agrocalendar', 'add', ['return' => $link_return]) ?>&amp;<?=time();?>" class="btn btn-default"><i class="fa fa-plus-circle" aria-hidden="true"></i> <?=CMS::t('menu_item_agrocalendar_add');?></a>
			<?php } ?>
		</nav>
	</section>
	...
	<section class="content">
		...
		<div class="box">
			<div class="box-header with-border">
				<h3 class="box-title"><?=CMS::t('agrocalendar_list_details', [
					'{count}' => $count,
				...
				<div class="box-tools pull-right col-sm-5 col-lg-6">
					<form action="" method="get" id="formSearchAndFilter">
						...
						<div class="input-group has-feedback">
							<input type="text" name="q" value="<?=utils::safeEcho(@$_GET['q'], 1);?>" placeholder="<?=CMS::t('agrocalendar_search');?>" class="form-control input-md" onfocus="this.value='';" />
            ...
            <div class="box-body">
				...
				<table class="table table-bordered table-striped">
					<thead>
						<tr>
							<th><?=CMS::t('agrocalendar_title');?></th>
					...
					<tbody id="tablesorter" data-page="<?=$current-1?>" data-limit="<?=$limit?>">
						...
						?>
							<tr data-id="<?=$data['id']?>">
							    <td><?=$data['title']?></td>
								<td>
									<?php if (CMS::hasAccessTo('agrocalendar/edit', 'write')) { ?>
										<a href="<?=view::create_url('agrocalendar', 'edit', ['id'=>$data['id']])?>" title="<?=CMS::t('edit');?>">
											<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
										</a>
									<?php } ?>

									<?php if (CMS::hasAccessTo('agrocalendar/delete', 'write')) { ?>
