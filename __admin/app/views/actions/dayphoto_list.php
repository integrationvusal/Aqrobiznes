 ...
 */
	...
	$('.tablesorter tbody').sortable({
		...
		stop: function(e, ui) {
			...
			$.ajax({
				url: '?controller=dayphoto&action=ajax_sort',
				async: false,
				cache: false,
				method: 'POST',
				data: {
...
<form action="?controller=dayphoto&amp;action=delete&amp;return=<?=$link_return;?>" method="post" id="formDeleteItem">
...
<section class="contextual-navigation">
	<nav>
		<?php if (CMS::hasAccessTo('dayphoto/add', 'write')) { ?>
		<a href="?controller=dayphoto&amp;action=add&amp;return=<?=$link_return;?>&amp;<?=time();?>" class="btn btn-default"><i class="fa fa-plus-circle" aria-hidden="true"></i> <?=CMS::t('menu_item_dayphoto_add');?></a>
		<?php } ?>
	</nav>
</section>
...
<section class="content">
	...
	<!-- <pre><?php /*var_export($dayphoto);*/ ?></pre> -->

	<div class="box">
		<div class="box-header with-border">
			<h3 class="box-title"><?=CMS::t('dayphoto_list_details', [
				'{count}' => $count,
			...
			<table class="table table-bordered table-striped tablesorter">
				<thead>
					<tr>
						...
						<th>#</th>
						<th><?=CMS::t('dayphoto_title');?></th>
						<th><?=CMS::t('dayphoto_author');?></th>
						<th><?=CMS::t('status');?></th>
				...
				<tbody>
					...
					?>
						<tr data-id="<?=$a['id'];?>" data-ordering="<?=$a['ordering'];?>">
							...
							<td>
								<?php if (CMS::hasAccessTo('dayphoto/ajax_set_status', 'write')) { ?>
								<a href="?controller=dayphoto&amp;action=ajax_set_status" title="" class="aAjax btn-toggle" data-ajax_post="<?=utils::safeEcho(json_encode([
									'CSRF_token' => $CSRF_token,
									'id' => $a['id'],
							...
							<td style="white-space: nowrap;">
								<?php if (CMS::hasAccessTo('dayphoto/edit', 'write')) { ?>
								<a class="a_i" href="?controller=dayphoto&amp;action=edit&amp;id=<?=$a['id'];?>&amp;return=<?=$link_return;?>&amp;<?=time();?>" title="<?=CMS::t('edit');?>">
									<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
								</a>
								<?php } else if (CMS::hasAccessTo('dayphoto/edit', 'read')) { ?>
								<a href="?controller=dayphoto&amp;action=edit&amp;id=<?=$a['id'];?>&amp;return=<?=$link_return;?>&amp;<?=time();?>" title="<?=CMS::t('view');?>">
								...
								<?php if (CMS::hasAccessTo('dayphoto/delete', 'write') && !isset($_GET['see_deleted'])) { ?>
...
<div id="popupFilter" style="display: none; width: 420px;">
	...
	<div class="popupForm">
		<div class="popupFormFieldset">
			...
			<div class="popupFormInputsBlock">
				<label class="form-label"><?=CMS::t('dayphoto_category');?></label>
				<div class="form-div-multicheckbox">
					<?php if (empty($allowed_cats)) { ?>
					<input type="checkbox" name="filter[cats][none]" value="1"<?=(isset($_GET['filter']['cats']['none'])? ' checked="checked"': '');?> id="multichoiceCat_None" form="formSearchAndFilter" /><label for="multichoiceCat_None"> <?=CMS::t('dayphoto_category_none');?></label><br />
					<?php } ?>
					<?php
						foreach ($cats as $c) {
