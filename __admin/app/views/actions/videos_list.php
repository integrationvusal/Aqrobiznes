...
$(document).ready(function() {
	...
	$('#tablesorter').sortable({
		...
		update: function(e, ui){
			...
			$.ajax({
				url: "<?=view::create_url('videos', 'ajax_sort')?>",
				async: false,
				cache: false,
				method: 'POST',
				data: {
...
// ]]>
	...
	<form action="<?=view::create_url('videos', 'delete') ?>" method="post" id="formDeleteItem">
	...
	<section class="contextual-navigation">
		<nav>
			<?php if (CMS::hasAccessTo('videos/add', 'write')) { ?>
			<a href="<?=view::create_url('videos', 'add', ['return' => $link_return]) ?>&amp;<?=time();?>" class="btn btn-default"><i class="fa fa-plus-circle" aria-hidden="true"></i> <?=CMS::t('menu_item_videos_add');?></a>
			<?php } ?>
		</nav>
	</section>
	...
	<section class="content">
		...
		<div class="box">
			<div class="box-header with-border">
				<h3 class="box-title"><?=CMS::t('videos_list_details', [
					'{count}' => $count,
				...
				<div class="box-tools pull-right col-sm-5 col-lg-6">
					<form action="" method="get" id="formSearchAndFilter">
						...
						<div class="input-group has-feedback">
							<input type="text" name="q" value="<?=utils::safeEcho(@$_GET['q'], 1);?>" placeholder="<?=CMS::t('videos_search');?>" class="form-control input-md" onfocus="this.value='';" />
            ...
            <div class="box-body">
				...
				<table class="table table-bordered table-striped">
					<thead>
						<tr>
							<th><?=CMS::t('videos_link_img');?></th>
							<th><?=CMS::t('videos_link');?></th>
							<th><?=CMS::t('videos_title');?></th>
					...
					<tbody id="tablesorter" data-page="<?=$current-1?>" data-limit="<?=$limit?>">
						<?php
							foreach ($alldata as $data) {
							    $data['link_img'] = '';
							    if($data['link'] && strpos($data['link'], 'v=') !== false){
    							    $data['link_img'] = explode('v=', $data['link']);
    							    $data['link_img'] = 'http://img.youtube.com/vi/'.$data['link_img'][1].'/mqdefault.jpg';
							    }
						?>
							<tr data-id="<?=$data['id']?>">
								<td><img width="100" src="<?=$data['link_img']?>"/></td>
								<td><?=$data['link']?></td>
								<td><?=$data['title']?></td>
								<td>
									<?php if (CMS::hasAccessTo('videos/edit', 'write')) { ?>
										<a href="<?=view::create_url('videos', 'edit', ['id'=>$data['id']])?>" title="<?=CMS::t('edit');?>">
											<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
										</a>
									<?php } ?>
									
									<?php if (CMS::hasAccessTo('videos/delete', 'write')) { ?>
