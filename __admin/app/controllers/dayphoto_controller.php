...
class dayphoto_controller extends controller {

	private static $runtime = [];

	public static function action_list() { 
		self::$layout = 'common_layout';
		view::$title = CMS::t('menu_item_dayphoto_list');
		...
		$params['news'] = news::getNewsList('dayphoto');
		...
		$params['canWrite'] = CMS::hasAccessTo('dayphoto/list', 'write');
		...
		return self::render('dayphoto_list', $params);
	}

	public static function action_add() { 
		self::$layout = 'common_layout';
		view::$title = CMS::t('menu_item_dayphoto_add');

		$params = [];

		$params['canWrite'] = CMS::hasAccessTo('dayphoto/add', 'write');
		$params['link_back'] = (empty($_GET['return'])? '?controller=dayphoto&action=list': $_GET['return']);
		...
		if (isset($_POST['add'])) {
			$params['op'] = news::addNews('dayphoto');
		...
		return self::render('dayphoto_add', $params);
	}

	public static function action_edit() { // 2016-01-15
		self::$layout = 'common_layout';
		view::$title = CMS::t('menu_item_dayphoto_edit');

		$params = [];

		$params['canWrite'] = CMS::hasAccessTo('dayphoto/edit', 'write');
		$params['link_back'] = (empty($_GET['return'])? '?controller=dayphoto&action=list': $_GET['return']);
		...
		return self::render('dayphoto_edit', $params);
	}
	...
	public static function action_delete() {
		...
		$params = [];
		$params['canWrite'] = CMS::hasAccessTo('dayphoto/delete', 'write');
		$params['link_back'] = (empty($_GET['return'])? '?controller=dayphoto&action=list': $_GET['return']);
	...
	public static function action_ajax_set_status() { // 2016-12-04
		...
		if (!CMS::hasAccessTo('dayphoto/ajax_set_status', 'write')) {
			$response['code'] = '403';
	...
	public static function action_ajax_sort() { // 2016-12-05
		...
		if (!CMS::hasAccessTo('dayphoto/ajax_sort', 'write')) {
			$response['code'] = '403';
	...
	public static function action_ajax_paged_sort() { // 2016-12-05
		...
		if (!CMS::hasAccessTo('dayphoto/ajax_paged_sort', 'write')) {
			$response['code'] = '403';
			$response['message'] = 'ajax_request_not_alllowed_to_write';
		} else if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])=='xmlhttprequest') {
	...
	public static function action_ajax_delete_image() { // 2016-12-05
		...
		if (!CMS::hasAccessTo('dayphoto/ajax_delete_image', 'write')) {
			$response['code'] = '403';
			$response['message'] = 'ajax_request_not_alllowed_to_write';
		} else if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])=='xmlhttprequest') {
