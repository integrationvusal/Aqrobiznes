...
class news {
	...
	public static function addNews($type = 'news') { // 2016-06-10
		...
		if (!utils::valid_date(@$_POST['publish_date'])) {
			...
			$news['publish_datetime'] = utils::formatPlainDate('Y-m-d', $_POST['publish_date']).' '.str_pad((int)$_POST['publish_hour'], 2, STR_PAD_LEFT).':'.str_pad((int)$_POST['publish_minutes'], 2, STR_PAD_LEFT).':'.date('s');
		}
		...
		if (!utils::valid_date(@$_POST['add_date'])) {
			...
			$news['add_datetime'] = utils::formatPlainDate('Y-m-d', $_POST['add_date']).' '.str_pad((int)$_POST['add_hour'], 2, STR_PAD_LEFT).':'.str_pad((int)$_POST['add_minutes'], 2, STR_PAD_LEFT).':'.date('s');
		}
		...
		if (!empty($_FILES['img']['name'])) {
			if (empty($_FILES['img']['error'])) {
				$_name = time().'.'.pathinfo($_FILES['img']['name'], PATHINFO_EXTENSION);

				$news['img'] = utils::upload($_name, $_FILES['img']['tmp_name'], UPLOADS_DIR.'news/originals/', self::$allowed_thumb_ext);
				if (empty($news['img'])) {
	...
	public static function editNews($id) { // 2016-06-10
		...
		$sef = trim(@$_POST['sef']);
		$againSef = CMS::$db->getRow("SELECT id FROM ".self::$tbl." WHERE sef = :sef", ['sef'=>$sef]);
		if(!empty($againSef) && $againSef['id'] != $id)	$response['errors'][] = 'news_add_err_sef_again';
		...
		if (!empty($_FILES['img']['name'])) {
			if (empty($_FILES['img']['error'])) {
				...
				$_name = time().'.'.pathinfo($_FILES['img']['name'], PATHINFO_EXTENSION);
				$uploaded = utils::upload($_name, $_FILES['img']['tmp_name'], UPLOADS_DIR.'news/originals/', self::$allowed_thumb_ext);
		...
		if (!utils::valid_date(@$_POST['publish_date'])) {
			...
			$upd['publish_datetime'] = utils::formatPlainDate('Y-m-d', $_POST['publish_date']).' '.str_pad((int)$_POST['publish_hour'], 2, STR_PAD_LEFT).':'.str_pad((int)$_POST['publish_minutes'], 2, STR_PAD_LEFT).':'.date('s');
		}
		...
		if (!utils::valid_date(@$_POST['add_date'])) {
			...
			$upd['add_datetime'] = utils::formatPlainDate('Y-m-d', $_POST['add_date']).' '.str_pad((int)$_POST['add_hour'], 2, STR_PAD_LEFT).':'.str_pad((int)$_POST['add_minutes'], 2, STR_PAD_LEFT).':'.date('s');
		}
