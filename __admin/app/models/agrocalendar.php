...
class agrocalendar{
	...
	public static $tbl = 'agrocalendar';
		...
		if(empty($response['errors'])){
			...
			if ($data_id) {
				CMS::log([
					...
					'subj_id' => $data_id,
					'action' => 'add',
					'descr' => 'agrocalendar '.$data_id.' added by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
				]);
		...
		if(empty($response['errors'])){
			...
			if ($updated) {
				CMS::log([
					...
					'subj_id' => $id,
					'action' => 'edit',
					'descr' => 'agrocalendar'.$id.' edited by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
				]);
    ...
    public static function delete($id){
		...
		if ($deleted) {
			...
			CMS::log([
				...
				'subj_id' => $id,
				'action' => 'delete',
				'descr' => 'agrocalendar deleted by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
			]);
		}
	}
		...
		foreach($items as $k=>$item){
			...
			if($sort){
				CMS::log([
					...
					'subj_id' => $item,
					'action' => 'ajax_sort',
					'descr' => 'agrocalendar sorted by '.$_SESSION[CMS::$sess_hash]['ses_adm_type'].' '.ADMIN_INFO,
				]);
			}
